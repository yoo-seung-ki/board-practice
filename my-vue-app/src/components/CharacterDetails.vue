...
const getCharacterClass = async (jobId, jobGrowId) => {
    ...
    const jobGroup = jobMapping[jobId];
    if (!jobGroup) {
        console.error('해당 직업군에 맞는 클래스를 찾을 수 없습니다.');
        return null;
    }

    const { dir, jobs } = jobGroup;
    const jobClassName = jobs[jobGrowId];
    if (!jobClassName) {
        console.error('해당 전직에 맞는 클래스를 찾을 수 없습니다.');
        return null;
    }

    const CharacterClass = await import(`../strategies/${dir}/${jobClassName}.js`)
        .then(module => module.default)
        .catch(err => {
            console.error('해당 클래스 파일을 로드하는 데 실패했습니다:', err);
            return null;
        });
